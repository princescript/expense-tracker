...
function isBadTitle(title: string): boolean {
  ...
  return (
    ...
    /^(after|before|had|paid|spent|received|credited|refund|for|around|hours|station|payment|salary|charged|app)$/i.test(normalized)
  );
}

export function generateSemanticFallback(category: string, type: string): string {
...
function parseTransactionRuleBased(input: string): {
  ...
  let finalCategory = "Others", maxCatScore = 0;
  for (const [cat, score] of Object.entries(categoryScores)) {
    if (score > maxCatScore) { maxCatScore = score; finalCategory = cat; }
  }
